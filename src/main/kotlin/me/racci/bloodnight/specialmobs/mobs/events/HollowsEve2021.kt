...
object HollowsEve2021 {
    ...
    fun attributeModifier(entity: LivingEntity, attribute: Attribute, amount: Double, operation: AttributeModifier.Operation = AttributeModifier.Operation.ADD_SCALAR) {
    ...
    init {
        ...
        entity.equipment.apply {
            helmet = ItemBuilder.from(Material.IRON_HELMET)
                ...
                .build()
            setItemInMainHand(ItemBuilder.from(Material.BOW)
...
class HollowHaunter(carrier: Phantom) : AbstractExtendedPhantom<Phantom, Stray>(carrier, SpecialMobUtil.spawnAndMount(carrier, HollowRider::class.java)) {
...
class HollowGoliath(entity: Husk) : AbstractZombie(entity) {
    ...
    init {
        ...
        entity.equipment.apply {
            ...
            setItemInMainHand(ItemBuilder.from(Material.DIAMOND_AXE)
...
class HollowNecromancer(entity: Drowned) : AbstractZombie(entity) {
    ...
    override fun onDamageByEntity(event: EntityDamageByEntityEvent) {
        if(tracker) {tracker = false ; return}
...
class HollowHarbinger(entity: WitherSkeleton) : AbstractWitherSkeleton(entity) {
    ...
    override fun tick() {
        ...
        if(now() - lastTeleport > 15) {
            if(baseEntity.location.distance(player.location) > 50.0) {lastDamager = null ; return}
            lastTeleport = now()
